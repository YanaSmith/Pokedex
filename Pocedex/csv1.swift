...
public class CSV {
    ...
    public init(content: String?, delimiter: NSCharacterSet, encoding: UInt) throws { 
        if let csvStringToParse = content{
            ...
            csvStringToParse.trimmingCharacters(in: newline).enumerateLines(invoking: { (line, stop) in
             lines.append(line)
            })//stringByTrimmingCharactersInSet(newline).enumerateLines { line, stop in lines.append(line) }
    ...
    func parseHeaders(fromLines lines: [String]) -> [String] {
        return lines[0].components(separatedBy: delimiter)//componentsSeparatedByCharactersInSet(self.delimiter)
    }
    ...
    func parseRows(fromLines lines: [String]) -> [Dictionary<String, String>] {
        ...
        for (lineNumber, line) in lines.enumerated() {
            ...
            let values = line.components(separatedBy: delimiter)//componentsSeparatedByCharactersInSet(self.delimiter)
